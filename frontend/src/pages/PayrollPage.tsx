  ...
  return (
    <Stack spacing={3}>
      ...
      <Typography variant="caption" color="text.secondary">
        ...
        Provident Fund, ESI, and a simplified Professional Tax slab. Running payroll for a month posts one journal
        entry — salary expense and the employer's own PF/ESI contribution debited, every statutory payable plus the
        net cash actually paid credited — and disburses net pay from the cash account you pick, same-day. Tag G/L
        accounts for <code>salary_expense</code>, <code>pf_payable</code>, <code>esi_payable</code>,{" "}
        <code>professional_tax_payable</code>, and <code>tds_payable</code> on the{" "}
        <code>Chart of Accounts</code> page before running payroll.
      </Typography>
      ...
      <Card variant="outlined">
        <CardContent>
          ...
          <Stack direction="row" spacing={2} sx={{ flexWrap: "wrap", alignItems: "center" }}>
            <TextField label="Name" size="small" value={empName} onChange={(e) => setEmpName(e.target.value)} sx={{ minWidth: 180 }} />
            <TextField label="PAN" size="small" value={empPan} onChange={(e) => setEmpPan(e.target.value)} sx={{ width: 140 }} />
            <TextField
            ...
            />
            <TextField select label="Tax regime" size="small" value={empRegime} onChange={(e) => setEmpRegime(e.target.value as TaxRegime)} sx={{ width: 130 }}>
              <MenuItem value="new">New regime</MenuItem>
              <MenuItem value="old">Old regime</MenuItem>
            </TextField>
            <TextField label="Basic / month" type="number" size="small" value={empBasic} onChange={(e) => setEmpBasic(e.target.value)} sx={{ width: 130 }} />
            <TextField label="HRA / month" type="number" size="small" value={empHra} onChange={(e) => setEmpHra(e.target.value)} sx={{ width: 130 }} />
            <TextField label="Special allowance" type="number" size="small" value={empSpecial} onChange={(e) => setEmpSpecial(e.target.value)} sx={{ width: 140 }} />
            <TextField label="Other allowance" type="number" size="small" value={empOther} onChange={(e) => setEmpOther(e.target.value)} sx={{ width: 140 }} />
            <FormControlLabel control={<Checkbox checked={empMetro} onChange={(e) => setEmpMetro(e.target.checked)} />} label="Metro city (HRA)" />
            <Button variant="contained" onClick={handleCreateEmployee} disabled={!empName || !empDoj || !empBasic}>
              Add employee
            </Button>
          </Stack>
        </CardContent>
      ...
      <Typography variant="caption" color="text.secondary">
        Only affects TDS for employees on the old regime — Section 80C (cap ₹1,50,000), Section 80D (cap ₹25,000),
        home loan interest under Section 24(b) (cap ₹2,00,000), and rent paid (feeds the HRA exemption calculation).
      </Typography>
      <Card variant="outlined">
        <CardContent>
          <Stack direction="row" spacing={2} sx={{ flexWrap: "wrap", alignItems: "center" }}>
      ...
      <Typography variant="h6">Form 16 (Annual TDS Certificate)</Typography>
      <Card variant="outlined">
        <CardContent>
          <Stack direction="row" spacing={2} sx={{ flexWrap: "wrap", alignItems: "center" }}>
