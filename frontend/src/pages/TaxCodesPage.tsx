  ...
  return (
    <Stack spacing={3}>
            ...
            <CardContent>
              ...
              <Grid container spacing={2}>
                ...
                <Grid size={{ xs: 6, sm: 4, md: 1.5 }}>
                  <TextField select label="Type" size="small" fullWidth value={taxType} onChange={(e) => setTaxType(e.target.value as TaxType)}>
                    {TAX_TYPES.map((t) => (
                ...
                <Grid size={{ xs: 12, sm: 6, md: 2 }}>
                  <TextField select label="Direction" size="small" fullWidth value={direction} onChange={(e) => setDirection(e.target.value as TaxDirection)}>
                    {DIRECTIONS.map((d) => (
                ...
                <Grid size={{ xs: 12, sm: 6, md: 4 }}>
                  <TextField select label="Tax G/L Account" size="small" fullWidth value={glAccountId} onChange={(e) => setGlAccountId(e.target.value)}>
                    {glAccounts.map((g) => (
              ...
              <Button variant="contained" sx={{ mt: 2 }} onClick={handleCreate} disabled={!country || !code || !name || !ratePct || !glAccountId}>
                Add tax code
              </Button>
            </CardContent>
          </Card>
      ...
      <TabPanel value={tab} index={2}>
        <Stack spacing={2}>
          <Typography variant="body2">
            The same approach SAP FI uses for multi-country tax: rather than hard-coding every country's tax law
            (which changes constantly and no small system can keep authoritative), set up one tax code per rate you
            actually deal with — a country, a percentage, and whether it's <strong>output</strong> tax you collect on
            sales (owed to the tax authority) or <strong>input</strong> tax you pay on purchases (usually
            recoverable).
          </Typography>
          <Typography variant="body2" color="text.secondary">
            Applying a tax code to a journal entry line on the{" "}
            <Link to="/general-ledger" style={{ color: "inherit" }}>
              Bookkeeping
            </Link>{" "}
            page auto-calculates and auto-posts the tax amount to the account you choose on the Tax Codes tab. India
            GST needing a CGST/SGST/IGST split instead of one flat account lives on its own page —{" "}
            <Link to="/gst-returns" style={{ color: "inherit" }}>
              GST Returns (India)
            </Link>
            .
          </Typography>
          <Typography variant="subtitle2">VAT / GST Return</Typography>
          <Typography variant="body2" color="text.secondary">
            Summed straight off the actual tax G/L postings for the period — the same way a real return is built off
            the tax account's activity, not off a separate calculation that could drift from what was actually
            posted. Net payable means you owe the tax authority; net refundable means your input tax exceeded your
