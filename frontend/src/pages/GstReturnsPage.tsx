  ...
  return (
    <Stack spacing={3}>
      ...
      <Typography variant="caption" color="text.secondary">
        A GST rate splits into CGST + SGST when the counterparty is in your own state, or IGST alone when they're
        not -- decided automatically by comparing your company's home state below against the customer's or vendor's
        state whenever you apply a rate on the{" "}
        <Link to="/receivables-payables" style={{ color: "inherit" }}>
          Receivables &amp; Payables
        </Link>{" "}
        page. Tag HSN/SAC codes on the{" "}
        <Link to="/chart-of-accounts" style={{ color: "inherit" }}>
          Chart of Accounts
        </Link>{" "}
        for the GSTR-1 HSN summary below.
      </Typography>
        ...
        <CardContent>
          ...
          <Stack direction="row" spacing={2} sx={{ flexWrap: "wrap", alignItems: "center" }}>
            <TextField
              ...
              sx={{ minWidth: 200 }}
            />
            <TextField label="Rate %" type="number" size="small" value={ratePct} onChange={(e) => setRatePct(e.target.value)} sx={{ width: 100 }} />
            <TextField select label="Direction" size="small" value={direction} onChange={(e) => setDirection(e.target.value as GstDirection)} sx={{ width: 150 }}>
              {DIRECTIONS.map((d) => (
            ...
            <TextField select label="CGST account" size="small" value={cgstAccount} onChange={(e) => setCgstAccount(e.target.value)} sx={{ minWidth: 180 }}>
              {glAccounts.map((g) => (
            ...
            <TextField select label="SGST account" size="small" value={sgstAccount} onChange={(e) => setSgstAccount(e.target.value)} sx={{ minWidth: 180 }}>
              {glAccounts.map((g) => (
                <MenuItem key={g.id} value={g.id}>
                  {g.code} {g.name}
                </MenuItem>
            ...
            <TextField select label="IGST account" size="small" value={igstAccount} onChange={(e) => setIgstAccount(e.target.value)} sx={{ minWidth: 180 }}>
              {glAccounts.map((g) => (
                <MenuItem key={g.id} value={g.id}>
                  {g.code} {g.name}
                </MenuItem>
            ...
            <Button variant="contained" onClick={handleCreate} disabled={!description || !ratePct || !cgstAccount || !sgstAccount || !igstAccount}>
              Add rate
            </Button>
          </Stack>
        </CardContent>
      ...
      <Typography variant="h6">GSTR-1 &amp; GSTR-3B</Typography>
      <Card variant="outlined">
        <CardContent>
          <Stack direction="row" spacing={2} sx={{ flexWrap: "wrap", alignItems: "center" }}>
            <TextField
