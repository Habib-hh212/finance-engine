...
React + TypeScript + Material UI, covering all Phase 1 backend modules: Dashboard (KPIs + AI Insights), Sales Forecasting, Budget Planning, Cash Flow Forecast, Cost Controlling & Variance, Profitability, and Financial Statements (Income Statement + Balance Sheet). Budget Planning also covers the Phase 2 Full Budget suite (Zero-Based, Flexible, Rolling, Capital) plus inline line editing/deleting and an expandable version-history trail on every submit. Profitability includes a Marginal Costing (CVP) section — fixed costs, break-even revenue, margin of safety, operating leverage. A dedicated Standard Costing page covers the standard cost sheet, production actuals, and the 8-variance method. Financial Statements includes a Phase 3 forecast section projecting the Income Statement and Balance Sheet forward (DSO/DPO/collection-lag inputs), plus a historical-trend method that projects straight from uploaded actuals history instead. Budget Planning's GL Account list lets you tag an account's forecast role (cash/AR/AP) to feed the driver-based forecast. Sales Forecasting's model dropdown includes two ML options (Random Forest, Gradient Boosting) alongside the statistical models, plus a "Compare models" panel showing walk-forward MAPE accuracy for every model side by side, and a "Demand prediction" panel forecasting units instead of revenue. Profitability adds a Customer Churn Risk table (recency/frequency ratio, not a trained classifier). A Scenario Planning page lets you create named what-if growth assumptions and compares the resulting Income Statement/Balance Sheet forecast against the base case side by side. The Dashboard's AI Insights list also includes statistical spend/sales anomalies (a z-score against each account's/product's own history), not just the fixed-threshold budget rules, plus two chart panels (revenue/expense/net-profit trend, budget vs. actual). Financial Statements and Sales Forecasting both support bulk Excel/CSV upload and "Download Excel" on every report. Cost Controlling adds Cost Center Accounting (tag actuals and budget lines to a responsibility unit, with its own variance report). Login/register gates the whole app (JWT). Sales Forecasting also has a Monte Carlo simulation panel (a p10/p50/p90 band that widens the further out the horizon goes, from 1,000 random trials — not the fixed-width CI on the base forecast). Financial Statements has a "Board Report" section for downloading a formatted PDF or PowerPoint. Two new pages: FX Scenario (record exchange rates, see the base-currency impact of a hypothetical rate move on non-base-currency sales) and Audit Trail (who changed what, when, filterable by entity type).
...
- No automated tests yet (backend has 127; frontend has none — component/integration tests would be the next addition)
