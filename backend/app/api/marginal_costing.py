...
def create_fixed_cost(company_id: uuid.UUID, payload: FixedCostIn, db: Session = Depends(get_db)):
...
def list_fixed_costs(company_id: uuid.UUID, fiscal_year: Optional[int] = None, db: Session = Depends(get_db)):
...
def get_marginal_costing_summary(company_id: uuid.UUID, fiscal_year: int, db: Session = Depends(get_db)):
