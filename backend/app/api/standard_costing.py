...
def upsert_standard_cost(payload: StandardCostIn, company_id: uuid.UUID = Depends(require_company_access), db: Session = Depends(get_db)):
    existing = (
...
def list_standard_costs(company_id: uuid.UUID = Depends(require_company_access), db: Session = Depends(get_db)):
...
def create_production_actual(payload: ProductionActualIn, company_id: uuid.UUID = Depends(require_company_access), db: Session = Depends(get_db)):
...
def list_production_actuals(company_id: uuid.UUID = Depends(require_company_access), db: Session = Depends(get_db)):
...
def get_standard_cost_variance(company_id: uuid.UUID = Depends(require_company_access), fiscal_year: Optional[int] = Query(None), db: Session = Depends(get_db)):
