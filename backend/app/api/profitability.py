...
def get_profitability_by_product(company_id: uuid.UUID = Depends(require_company_access), db: Session = Depends(get_db)):
...
def get_profitability_by_customer(company_id: uuid.UUID = Depends(require_company_access), db: Session = Depends(get_db)):
...
def get_customer_churn_risk(company_id: uuid.UUID = Depends(require_company_access), as_of: Optional[date] = Query(None), db: Session = Depends(get_db)):
